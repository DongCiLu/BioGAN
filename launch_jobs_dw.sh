...
# `infogan`.
gan_type=$1
if ! [[ "$gan_type" =~ ^(unconditional|conditional|infogan|multiple|medium|classification|temp|tiny|tinygan) ]]; then
...
# Run classifier
if [[ "$gan_type" == "medium" ]]; then
  VERSION_NUMBER="raw_1test_64"
  CLASSIFICATION_DATASET_DIR="celegans-64-supervised"
  ...
  # Run training.
  Banner "Starting training celegans classifier for ${NUM_STEPS} steps..."
  python "${git_repo}/BioGAN/classification/train_dw.py" \
    --train_log_dir=${CLASSIFICATION_TRAIN_DIR} \
    --dataset_dir=${CLASSIFICATION_DATASET_DIR} \
    --data_config="full_1to3" \
    --hyper_mode="medium" \
    --network="dconvnet" \
    --max_number_of_steps=${NUM_STEPS} \
    --warm_start=0
